...
namespace Z.MyToDoWebAPI.Context.UnitOfWork
{
    /// <summary>
    ...
    public static class IEnumerablePagedListExtensions
    {
        ...
        /// <param name="indexFrom">The start index value.开始索引值</param>
        /// <returns>An instance of the inherited from <see cref="IPagedList{T}"/> interface.接口继承的实例</returns>
        public static IPagedList<TResult> ToPagedList<TSource, TResult>(this IEnumerable<TSource> sources, Func<IEnumerable<TSource>, IEnumerable<TResult>> converter, int pageIndex, int pageSize, int indexFrom = 0) => new PagedList<TSource, TResult>(sources, converter, pageIndex, pageSize, indexFrom);
    }
}
